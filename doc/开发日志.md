...
### 2024-12-11: 计算引擎 Bug 调查与讨论

#### 问题描述

外部 Review 提出了两个潜在 Bug：

**Bug 1: `crit_zone` 计算公式**

- **问题描述**：`build_multiplier_breakdown` 中 `crit_zone` 使用 `1.0 + effective_crit_chance * crit_multiplier`，但 `crit_multiplier` 可能已包含基础 1.0x，导致暴击期望区被重复计算。
- **对比 `calculate_crit_factor`**：该函数使用 `1.0 + crit_chance * (crit_multiplier - 1.0)`。
- **讨论**：需要确认 `crit_multiplier` 的语义是"总暴击伤害倍率"还是"额外暴击伤害"。
  - 如果 `crit_multiplier = 1.5` 表示 150% 暴击伤害，则应使用 `(crit_multiplier - 1.0)`
  - 如果 `crit_multiplier = 0.5` 表示额外 50% 暴击伤害，则当前公式正确

**Bug 2: 缓存键缺失 `context_flags` 和 `context_values`**

- **问题描述**：`CacheKey` 未包含 `context_flags`（如 `cannot_crit`）和 `context_values`（如 `enemy_range`），导致不同上下文配置可能产生相同缓存键。
- **影响**：悬停预览时可能返回错误的缓存结果。
- **建议修复**：在 `CacheKey` 中添加 `context_flags_hash` 和 `context_values_hash` 字段。

#### 待办事项

- [ ] 确认 `crit_multiplier` 的语义定义，统一全代码库使用
- [ ] 评估 `context_flags/values` 是否影响缓存键（取决于是否有条件计算依赖这些值）
- [ ] 添加相关单元测试验证边界情况
