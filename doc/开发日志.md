...
### 2025-12-14: 单元测试回归与暴击配置修复

#### 背景
- 复现场景：“闪电链 + 闪转冰冷 + 灵能乍泄 + 伊斯拉菲尔旧律 + 苦寒/积聚/世事无常”。
- 与基准文档《单元测试结果与乘区溯源.md》的正确结果出现偏差：暴击率偏低、暴击伤害偏高。

#### 问题根因
- 单测输入误写：
  - `crit.chance = 0.075`（应为 10%）。
  - 额外注入 `crit.dmg = 0.50`，叠加到引擎内置的基础 150% 暴击伤害，导致暴击伤害虚高。
- 引擎本身暴击逻辑正确，属测试用例配置错误。

#### 修复
- 将暴击率恢复为 10%（换算自：基础法术暴击值 500 × 战意 100 层，每层 +2% 暴击值）。
- 移除多余的 `crit.dmg` 覆写，保持基础暴击伤害 150% + 旧律 6 层暴伤（24%）。
...
More Zone: 5.6754
Crit Zone: 1.0740
伤害类型: cold = 8092.08, history tags ["tag_22","tag_23"]
```
- 与基准文档数值完全一致，偏差已消除。

#### 经验
- 引擎已内置基础 150% 暴击伤害，`global_overrides` 中无需再注入。
- 保持暴击值换算公式一致：`crit_chance = crit_value / 20000`（本用例直接写入换算后的 10%）。

---

### 2025-12-15: 法术迸发触发模式开关 + 回归

#### 变更
- 新增上下文开关 `context_flags.use_spell_burst`：
  - 开启时：若 M<1 或资格不符 → rate=0（DPS=0）；否则 `rate = M / t_full`（不再取 0.1s 间隔）。
  - 关闭时：保持原有迸发逻辑（`M / max(t_full, (M-1)*0.1)`，不丢伤害）。
- 重构充能参数计算为 `compute_spell_burst_charge_params`，复用资格判定 + PlaySafe 施法速度叠加充能。
- 单测新增：
  - M<1 + use_spell_burst=true → rate=0, DPS=0
  - M=3, t_full=2 → rate=1.5/s
  - 链闪全量场景（开启迸发触发模式）

#### 链闪迸发触发场景（开启 use_spell_burst）
- 配置：链闪21 + 闪转冰20 + 灵能乍泄20 + 旧律 + 苦寒/积聚/世事无常 + Beacon(+2迸发层) + PlaySafe + 聚能祝福6层 + 战意100层，施法速度 +100%（global）+16%（灵能乍泄）。
- 结果：`DPS = 18772.33`，`hit = 8690.89`，`rate = 2.16/s`，`crit = 10% / 1.74x`。
- 乘区：`base=1425.82, inc=1.0, more=5.6754, crit=1.0740, speed=2.1600, hit=0.95`，其他区均为 1。
- 伤害：cold=8092.08，历史标签保留 lightning。

#### rate=2.16/s 计算拆解
- M=2（Beacon +2），迸发充能加速因子 `denom = (1 + inc) * more`。
- 施法速度 inc：`speed.cast = 1.0`（+100%）+ 灵能乍泄 0.16 → inc=1.16；more=1。
- PlaySafe 叠加施法速度到充能 → `denom = (1+1.16)*1 = 2.16`。
- `t_full = 2 / 2.16 ≈ 0.9259s`，`rate = M / t_full = 2 / 0.9259 ≈ 2.16/s`。

---

### 2025-12-15: 法术迸发机制与数据联动

#### 变更概览
- **数据层 (Supabase Seed)**：新增法术迸发相关元数据  
  - `attributes_meta`: `speed.spell_burst_charge` / `mod.more.speed.spell_burst_charge` / `mechanic.spell_burst.max_stacks` / `flag.talent.playsafe`  
  - `tags_registry`: `Mech_SpellBurst`  
  - `mechanics_registry`: `spell_burst`（默认上限 0）  
  - `hero_traits`: `trait_playsafe`（施法速度作用于迸发充能） / `trait_beacon`（+2 迸发上限）
- **标签映射**：`tli-core/src/data/tags_registry.json` 同步加入 `Mech_SpellBurst`。
- **引擎管线**：在 `pipeline.rs` 新增 `compute_spell_burst_rate`，对符合资格的法术用 `rate_burst = M / max(t_full, (M-1)*0.1)` 替换原速率；`t_full = 2/(1+inc)*more`，PlaySafe 叠加施法速度 inc/more；输出调试 trace。
- **资格判定**：仅对无冷却的法术生效，排除吟唱/召唤/图腾/陷阱/烙印标签。

#### 测试
- 新增单测 5 项覆盖：`M=0` 不生效、充能瓶颈/间隔瓶颈、PlaySafe 加速、冷却拦截。
- `cargo test spell_burst` 通过（仍有既有 ts-rs serde 默认值警告，不影响结果）。

#### 待上线动作
- 若需落地到实际 Supabase 项目，请执行对应 UPSERT 脚本（seed 已包含最新数据）。
